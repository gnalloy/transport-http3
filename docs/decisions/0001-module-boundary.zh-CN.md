# ADR-001：保持 transport-http3 为独立 Gnalloy 模块
...
`transport-http3` 作为 `gnalloy.org/transport-http3` 发布，并拥有该职责：HTTP/3 传输绑定，将 QUIC request/control/QPACK/push stream 映射到 Gnalloy Channel Pipeline。
