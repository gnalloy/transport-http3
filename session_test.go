...
func TestSessionOpenPushStreamWritesTypeAndPushID(t *testing.T) {
	conn := newFakeConnection()
	session, err := NewSession(conn, Config{})
	if err != nil {
		t.Fatal(err)
	}
	if _, err := session.OpenPushStream(context.Background(), 9); err != nil {
		t.Fatal(err)
	}
	if got := conn.openedUni.written.Bytes(); !bytes.Equal(got, []byte{byte(codechttp3.StreamTypePush), 9}) {
		t.Fatalf("push prefix=%v", got)
	}
}

func TestSessionAcceptPushStreamReadsPushID(t *testing.T) {
	conn := newFakeConnection()
	conn.acceptedUni.feed([]byte{byte(codechttp3.StreamTypePush), 5, byte(codechttp3.FrameHeaders), 0})
	session, err := NewSession(conn, Config{})
	if err != nil {
		t.Fatal(err)
	}
	streamCh, err := session.AcceptPushStream(context.Background())
	if err != nil {
		t.Fatal(err)
	}
	capture := &captureInbound{}
	...
	id, ok := capture.messages[1].(codechttp3.PushIDFrame)
	if !ok || id.PushID != 5 {
