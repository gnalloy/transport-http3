...
HTTP/3 传输绑定，将 QUIC request/control/QPACK/push stream 映射到 Gnalloy Channel Pipeline。

该模块负责一个具体 I/O 边界，为具体传输创建或适配 Gnalloy Channel。协议解析、业务 handler、TLS 策略和观测能力由其他模块负责。

## 状态

- 导入路径：`gnalloy.org/transport-http3`
- 仓库：`github.com/gnalloy/transport-http3`
- 默认分支：`dev`
- 预览安装：`go get gnalloy.org/transport-http3@dev`
- 许可证：Apache-2.0
...
本仓库负责：HTTP/3 传输绑定，将 QUIC request/control/QPACK/push stream 映射到 Gnalloy Channel Pipeline。
